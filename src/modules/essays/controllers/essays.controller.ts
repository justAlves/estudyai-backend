...
export const essaysController = new Elysia({ prefix: "/essays", tags: ["Essays"] }).use(accessControl)
  ...
  .post("/", async ({ request, headers, jwt, set }) => {
    ...
    try {
      ...
      try { await enqueueEssayGeneration(jobId); } catch (error) { apiLogger.error({ err: error, essayId, jobId }, "falha ao enfileirar correção de redação"); await db.transaction(async (tx) => { await tx.update(essays).set({ status: "FAILED" }).where(eq(essays.id, essayId)); await tx.update(essayGenerationJobs).set({ status: "FAILED", errorMessage: "Não foi possível colocar a redação na fila." }).where(eq(essayGenerationJobs.id, jobId)); }); set.status = 503; return { message: "O corretor está temporariamente indisponível. Tente novamente em alguns instantes." }; }
